...
// Global pool for connection reuse across serverless invocations
...
function getPool(): Pool {
  if (!globalPool) {
    ...
    if (!connectionString) {
      throw new Error('DATABASE_URL environment variable is required for Supabase');
    }

    globalPool = new Pool({
      connectionString,
      max: 5, // Lower for serverless to avoid connection exhaustion
